...
test('client.interaction.terms-and-conditions :: accept', async ({ page }) => {
  ...
  await page.evaluate(() => (window as any).__aptixClientInteractionTest?.load({ directiveId: 'TEST-DIRECTIVE-TERMS-001', action: 'client.terms-and-conditions', termsAndConditionsVersion: 'TEST-TERMS-V1' }));
  ...
  const completion = await page.evaluate(() => (window as any).__aptixClientInteractionTest?.getCompletion());
