...
test('client.interaction.terms-and-conditions :: accept', async ({ page }) => {
  ...
  await page.evaluate(() => (window as any).__aptixClientInteractionTest?.load({ sessionId: 'TEST-SESSION', turnId: 'TEST-TURN', clientDirectives: [{ directiveId: 'TEST-DIRECTIVE-TERMS-001', action: 'client.terms-and-conditions' }], termsAndConditionsVersion: 'TEST-TERMS-V1' }));
  ...
  await expect(page.getByTestId('client-interaction-test-request')).toContainText('clientDirectiveResults');
  const submittedRequest = await page.evaluate(() => (window as any).__aptixClientInteractionTest?.getSubmittedRequest());
  expect(submittedRequest).toEqual({ sessionId: 'TEST-SESSION', turnId: 'TEST-TURN', clientDirectiveResults: [{ directiveId: 'TEST-DIRECTIVE-TERMS-001', action: 'client.terms-and-conditions', result: 'accepted' }] });
});
