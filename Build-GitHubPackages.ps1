...
$solutionPath = Join-Path $repoRoot 'UserAdmin.sln'
...
$nuspecFiles = @(
    $packageRoots |
        ForEach-Object {
            ...
            if (Test-Path $root) { Get-ChildItem -Path $root -Filter 'Package.nuspec' -File -Recurse }
        } |
        Sort-Object FullName
)
if ($nuspecFiles.Count -eq 0) { throw 'No Package.nuspec files were found beneath src or tests.' }
...
foreach ($package in $packages) {
    ...
    foreach ($file in @($package.Xml.SelectNodes('//files/file'))) {
        if (-not [string]::IsNullOrWhiteSpace([string]$file.src)) {
            ...
            for ($i = 0; $i -lt $segments.Count; $i++) {
                if ($segments[$i] -ieq 'release') { $segments[$i] = 'Release' }
                elseif ($segments[$i] -match '^(?i:net(?:standard)?\d)') { $segments[$i] = $segments[$i].ToLowerInvariant() }
            }
    ...
    try { $package.Xml.Save($writer) }
    finally { $writer.Dispose() }
}

Write-Host "Restoring UserAdmin.sln for package set $Version..."
dotnet restore $solutionPath --configfile (Join-Path $repoRoot 'NuGet.config')
if ($LASTEXITCODE -ne 0) { throw "dotnet restore failed with exit code $LASTEXITCODE." }

Write-Host 'Building UserAdmin.sln...'
...
foreach ($package in ($packages | Sort-Object Id)) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "dotnet pack failed for '$($package.Id)' with exit code $LASTEXITCODE." }
...
$sourceRepository = if ($env:GITHUB_REPOSITORY) { $env:GITHUB_REPOSITORY } else { 'LagoVista/UserAdmin' }
