...
namespace LagoVista.UserAdmin.Tests.EmailIntegrationTests
{
    ...
    public class EmailAPI
    {
        ...
        {
            var result = await _emailSenderService.AddEmailDesignAsync($"TESTING - {DateTime.Now}", "SEND THE EMAIL", "HTML CONTENTS", "PLAIN TEXT CONTENTS");
        ...
        public async Task UpdateDesignAsync()
        {
            ...
            var updateResult = await _emailSenderService.UpdateEmailDesignAsync(result.Result, "UPDATED TESTING", "UPDATE SUBJECT", "UPDATE HTML CONTENT", "UPDATE TEXT CONTENTS");
        ...
        public async Task AddSenderAsync()
        {
            ...
            var result = await _emailSenderService.AddEmailSenderAsync(appUser);
            Assert.IsTrue(result.Successful, result.ErrorMessage);
            ...
            var deleteResult = await _emailSenderService.DeleteEmailSenderAsync(appUser.SendGridSenderId);
        ...
        public async Task UpdateSenderAsync()
        {
            ...
            result = await _emailSenderService.UpdateEmailSenderAsync(appUser);
