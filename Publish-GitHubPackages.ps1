...
$basicCredential = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes("$($env:NUGET_GITHUB_USERNAME):$($env:NUGET_GITHUB_TOKEN)"))
$headers = @{
    Authorization = "Basic $basicCredential"
    'User-Agent' = 'softwarelogistics-build-server/0.1'
}

$serviceIndex = Invoke-RestMethod -Uri $packageSource -Headers $headers -Method Get
...
if ($null -eq $packageBaseResource -or [string]::IsNullOrWhiteSpace([string]$packageBaseResource.'@id')) {
    throw "GitHub Packages service index does not expose PackageBaseAddress: $packageSource"
}
