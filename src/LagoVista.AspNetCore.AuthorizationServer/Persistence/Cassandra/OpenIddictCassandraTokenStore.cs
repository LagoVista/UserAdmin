...
namespace LagoVista.AspNetCore.AuthorizationServer.Persistence.Cassandra
{
    ...
    /// </summary>
    public sealed class OpenIddictCassandraTokenStore : IOpenIddictTokenStore<OpenIddictTableToken>
    {
...
USING TTL ?").ConfigureAwait(false);
        ...
        public IAsyncEnumerable<OpenIddictTableToken> FindAsync(string subject, string client, string status, string type, CancellationToken cancellationToken)
        ...
        public async ValueTask<TResult> GetAsync<TState, TResult>(Func<IQueryable<OpenIddictTableToken>, TState, IQueryable<TResult>> query, TState state, CancellationToken cancellationToken)
        {
            if (query == null) throw new ArgumentNullException(nameof(query));
