...
namespace LagoVista.UserAdmin.Repos.Relational
{
    ...
    public class OrganizationRelationalRepo : RelationalBase<BillingDataContext>, IOrganizationRelationalRepo
    {
        ...
        public OrganizationRelationalRepo(IDbContextFactory<BillingDataContext> context, IAdminLogger adminLogger, ILagoVistaAutoMapper autoMapper, ISecureStorage secureStorage) :
        ...
        public Task<bool> DoesOrgHaveBillingRecords(NormalizedId32 orgId, EntityHeader org, EntityHeader user)
        {
            return WithContextAsync(org, user, async ctx =>
            {
                return await ctx
                .BillingEvents
                .Where(be => be.Subscription.OrganizationId == orgId)                
                .AnyAsync();
            });
        }
